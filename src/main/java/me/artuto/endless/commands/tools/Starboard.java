...
public class Starboard extends Command
{
    ...
    private void waitForChannel(CommandEvent event)
    {
        ...
        ew.waitForEvent(GuildMessageReceivedEvent.class,
                ...
                e -> {
                    switch(e.getMessage().getContentRaw().split(" ", 2)[0].toLowerCase())
                    {
                        ...
                        case "create":
                            ...
                            event.replySuccess("Ok, this will create a channel named **\"starboard\"**, you can change the name later.",
                                    m -> m.editMessage("Starting setup...").queueAfter(3, TimeUnit.SECONDS, (s) -> {
                                        event.getGuild().getController().createTextChannel("starboard").queue(everyone ->
                                                everyone.createPermissionOverride(event.getGuild().getPublicRole()).setDeny(Permission.MESSAGE_WRITE).queue(self ->
                                                        self.getChannel().createPermissionOverride(event.getSelfMember()).setAllow(Permission.MESSAGE_WRITE).queue(next -> {
                        ...
                        case "created":
                            ...
                            if(list.isEmpty())
                                event.replyWarning("No Text Channels found matching \""+tc+"\"");
                            else if (list.size()>1)
                ...
                2, TimeUnit.MINUTES, () -> event.replyWarning("Oh uh.... You took more than 2 minutes to answer "+
                        event.getAuthor().getAsMention()+"! Cancelling setup.")
        );
    }
