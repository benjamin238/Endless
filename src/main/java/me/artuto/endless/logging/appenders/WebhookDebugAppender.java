...
public class WebhookDebugAppender extends AppenderBase<ILoggingEvent>
{
    private WebhookClient client;

    public WebhookDebugAppender() throws IOException
    {
        List<String> config = Files.readAllLines(Paths.get("webhook.txt"));
        client = new WebhookClientBuilder(config.get(1)).setDaemon(true).build();
    }
    ...
    protected void append(ILoggingEvent event)
    {
        ...
        eBuilder.setDescription(MiscUtils.getStackTrace(event));
